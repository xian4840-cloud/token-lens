...
/**
 * 扫描本地四家 agent，按 model+date 聚合，用价格表（含设置页覆盖）换算费用。
 * - OpenCode 自带 cost，优先用其值，不经过 computeCost。
 * - Claude Code / Codex / Antigravity 用 computeCost 补算；价格表匹配不到的模型 cost 为 undefined。
...
export async function scanLocalUsage(since?: string): Promise<ScanLocalUsageResult> {
  ...
  const [claudeRows, codexRows, opencode, antigravity] = await Promise.all([
